...
# + Try to make predictions based on how much rain falls, how high the creek is
# going to be for how long
#     -Then look at the weather forecast and predict (based on where the creek
# is now) how high it will be in the future
...
#
# Obviously snow melt is a thing. The NOAA provides snow melt data that
# we could incoroporate
# or we could only look at data where all the snow has melted and look at how
# much the creek
...
c.execute('''
CREATE TABLE IF NOT EXISTS CreekDaily ("Date" DATE PRIMARY KEY,
                                       FlowRate REAL, Temp REAL)''')
...
# User defined specific dates. For whatever reason there is a gap in data
...
if days:
    payload = {
        'format': 'json', 'indent': 'on', 'sites': '05289800',
        'period': 'P' + str(days) + 'D', 'siteStatus': 'all'
        }
else:
    payload = {
        'format': 'json', 'indent': 'on', 'sites': '05289800',
...
for metric in metrics:
    if "Temp" in data['value']['timeSeries'][metric]['variable']['variableDescription']:
        for day in range(len(data['value']['timeSeries'][metric]['values'][0]['value'])):
            ...
            c.execute('REPLACE INTO CreekDaily ("Date", Temp) VALUES (?,?)',
                      (Date[:10], Temp))
    else:
        for day in range(len(data['value']['timeSeries'][metric]['values'][0]['value'])):
            Date = data['value']['timeSeries'][metric]['values'][0]['value'][day]['dateTime']
            ...
            c.execute('UPDATE CreekDaily SET FlowRate = ? WHERE "Date" = ?',
                      (FlowRate, Date[:10]))
